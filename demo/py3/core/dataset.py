...
conn = web.HttpConnection("localhost", 44367)

weapons = []
try:
    make_weapon = lambda t: battle.Weapon(Id=t["Id"], name=t["Name"], damage=t["Damage"], typeRange=t["Type"], price=t["Price"], d=t["Far"], n=t["Near"])
    weapons = conn.upload("/api/weapon/", make_weapon)
except Exception:
...
shields = []
try:
    make_armor = lambda a: battle.Shield(Id=a["Id"], name=a["Name"], squer=a["Level"], resource=a["Resource"])
    shields = conn.upload("/weapons/armor/", make_armor)
except Exception:
...
warriors = []
try:
    make_men = lambda w: battle.SoldierProfile(Id=w["Id"], name=w["Name"], weapon=make_weapon(w["Malee"]), firetool=make_weapon(w["Range"]), shield=make_armor(w["Armor"]), transport=w["Speed"], resource=w["Resource"])
    warriors = conn.upload("/api/soldier/classes", make_men)
except Exception:
...
mashines = []
try:
    machines = conn.upload("/api/soldiers/machines", \
    lambda m: battle.SeigeMachine(Id=m["Id"], name=m["Name"], distance=m["Far"], near=m["Near"], crashValue=m["HP"], resource=m["Range"]["Resource"], sequence=m["ShotPerSec"], fier=make_weapon(m["Tool"])))
except Exception:
...
landscapes = []
try:
    landscapes = conn.upload("/api/lanscape", \
    lambda l: world.Landscape(Id=l.Id, name=l.Name, cross=l.Cross))
except Exception:
...
def upload(conn, api_func, convert):
    conn.request("GET", api_func)
